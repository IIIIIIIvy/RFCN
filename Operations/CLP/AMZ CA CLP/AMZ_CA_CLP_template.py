...
CLP_DICT = {"<28": "CFS", "<56": "20GP", "<65": "40GP", "<75": "40HQ", ">=75": "45HQ"}

EDGE_CLP = {"CFS": 0, "20GP": 28, "40GP": 56, "40HQ": 65, "45HQ": 75}


def splitCLP(ttl_cbm, temp_res):
    if ttl_cbm >= 75:
        temp_res += CLP_DICT[">=75"]
        ttl_cbm -= 75
    elif ttl_cbm < 75 and ttl_cbm >= 65:
        temp_res += CLP_DICT["<75"]
        ttl_cbm -= 65
    elif ttl_cbm < 65 and ttl_cbm >= 56:
        temp_res += CLP_DICT["<65"]
        ttl_cbm -= 56
    elif ttl_cbm < 56 and ttl_cbm >= 28:
        temp_res += CLP_DICT["<56"]
        ttl_cbm -= 28
    else:
        temp_res += CLP_DICT["<28"]
        ttl_cbm -= 28
    ...
    for name, group in groupBy:
        ...
        if len(temp) > 1:
            ...
            for flag in overlap_list:
                ...
                groupIndex += 1
                # print(data.loc[temp[temp['overlap']==flag].index][['Vendor Name','Vendor code','FC','First Ship Date','Last Ship Date','groupIndex']])
                # print()
        else:
...
def data_extraction(file_dir, data_file_name):
    ...
    data_isin_gxd = shipping_window_overlap_calculation(data_isin_gxd)
    ...
    data_notin_gxd = shipping_window_overlap_calculation(data_notin_gxd)
    ...
    for vendor in vendor_list:
        ...
        if len(df1) != 0:
            for index in df1["groupIndex"].unique():
                ...
                worksheet["A" + str(writer_pointer)] = "AMA841N"
                    ...
                    for edge in edge_list:
                        ...
                        if edge == 0:
                            ...
                            if split_flag:
                                ...
                                # split_item_list.append(writer_pointer+row_pointer+1)
                        ...
                        while row_pointer < len(groupdf):
                            ...
                            if temp_sum + current_cbm < edge:
                                ...
                                groupdf.loc[start_row:row_pointer, "CLP"] = (
                                    split_res_list[edge_list.index(edge)]
                                )
                                ...
                                if split_flag:
                                    ...
                                    # 两个非CFS的要拼接公式
                                    writer_pointer += 1
                                    formula_add_dict[writer_pointer] = [
                                        formula_list[-2],
                                        formula_list[-1],
                                    ]
                                ...
                                # !!Section:row_pointer到这一行发现加起来超过了edge，需要进行split，但这个frame还没有被spilt过，则是这一单的开头frame
                                    ...
                                    split_item_PO = groupdf.loc[
                                        row_pointer, "Customer PO"
                                    ]
                                    split_item_ASIN = groupdf.loc[
                                        row_pointer, "ASIN# or SKU#... "
                                    ]

                                    groupdf[start_row : row_pointer + 1] = groupdf[
                                        start_row : row_pointer + 1
                                    ].sort_values(["Customer PO", "ASIN# or SKU#... "])
                                    groupdf[start_row : row_pointer + 1].reset_index(
                        ...
                        if write_flag:
                            ...
                            groupdf.loc[start_row:row_pointer, "CLP"] = split_res_list[
        ...
        for item in formula_add_dict:
            worksheet["I" + str(item)] = (
                "=SUM(I"
                + str(formula_add_dict[item][0])
                + ",I"
                + str(formula_add_dict[item][1])
                + ")"
            )
            worksheet["Q" + str(item)] = (
                "=SUM(Q"
                + str(formula_add_dict[item][0])
                + ",Q"
                + str(formula_add_dict[item][1])
...
if __name__ == "__main__":
    ...
    # print(folder_path)
    data_file_name = "AMZ CA Cover.xlsx"
